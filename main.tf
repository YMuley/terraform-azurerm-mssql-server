resource "azurerm_mssql_server" "sql_server"{
    ...
    administrator_login          = sql_server.value.azuread_administrator.azuread_authentication_only == true ? null : each.value.administrator_login_name
    administrator_login_password = sql_server.value.azuread_administrator.azuread_authentication_only == true ? null : each.value.administrator_login_password
    minimum_tls_version          = each.value.minimum_tls_version
    public_network_access_enabled = each.value.public_network_access_enabled


    dynamic "azuread_administrator" {
      for_each = each.value.administrator_login_name != null || length(each.value.administrator_login_name) != "0" ? [] : sql_server.value.azuread_administrator
      content {
    ...
    dynamic "identity" {
      for_each = sql_server.value.azuread_administrator
      content {
        type = "UserAssigned"
    ...
    primary_user_assigned_identity_id            = var.user_assigned_identity_output[sql_server.value.azuread_administrator.login_username].id
